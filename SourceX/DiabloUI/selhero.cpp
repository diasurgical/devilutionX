...
UiItem SELHERO_DIALOG[] = {
	...
	UiArtText(N_("Level:"), { 39, 323, 110, 21 }, UIS_RIGHT),
	UiArtText(textStats[0], { 159, 323, 40, 21 }, UIS_CENTER),
	UiArtText(N_("Strength:"), { 39, 358, 110, 21 }, UIS_RIGHT),
	UiArtText(textStats[1], { 159, 358, 40, 21 }, UIS_CENTER),
	UiArtText(N_("Magic:"), { 39, 380, 110, 21 }, UIS_RIGHT),
	UiArtText(textStats[2], { 159, 380, 40, 21 }, UIS_CENTER),
	UiArtText(N_("Dexterity:"), { 39, 401, 110, 21 }, UIS_RIGHT),
	UiArtText(textStats[3], { 159, 401, 40, 21 }, UIS_CENTER),
	UiArtText(N_("Vitality:"), { 39, 422, 110, 21 }, UIS_RIGHT),
...
UiItem SELLIST_DIALOG[] = {
	UiArtText(N_("Select Hero"), { 264, 211, 320, 33 }, UIS_CENTER | UIS_BIG),
	UiList(SELLIST_DIALOG_ITEMS, 265, 256, 320, 26, UIS_CENTER | UIS_MED | UIS_GOLD),
	MakeScrollBar({ 585, 244, 25, 178 }),
	UiArtTextButton(N_("OK"), &UiFocusNavigationSelect, { 239, 429, 120, 35 }, UIS_CENTER | UIS_BIG | UIS_GOLD),
	UiArtTextButton(N_("Delete"), &selhero_UiFocusNavigationYesNo, { 364, 429, 120, 35 }, UIS_CENTER | UIS_BIG | UIS_DISABLED),
	UiArtTextButton(N_("Cancel"), &UiFocusNavigationEsc, { 489, 429, 120, 35 }, UIS_CENTER | UIS_BIG | UIS_GOLD)
};
...
UiListItem SELCLAS_DIALOG_ITEMS[] = {
	{ N_("Warrior"), UI_WARRIOR },
	{ N_("Rogue"), UI_ROGUE },
	{ N_("Sorcerer"), UI_SORCERER }
};
UiItem SELCLASS_DIALOG[] = {
	UiArtText(N_("Choose Class"), { 264, 211, 320, 33 }, UIS_CENTER | UIS_BIG),
	UiList(SELCLAS_DIALOG_ITEMS, 264, 285, 320, 33, UIS_CENTER | UIS_MED | UIS_GOLD),
	UiArtTextButton(N_("OK"), &UiFocusNavigationSelect, { 279, 429, 140, 35 }, UIS_CENTER | UIS_BIG | UIS_GOLD),
	UiArtTextButton(N_("Cancel"), &UiFocusNavigationEsc, { 429, 429, 140, 35 }, UIS_CENTER | UIS_BIG | UIS_GOLD)
};

UiItem ENTERNAME_DIALOG[] = {
	UiArtText(N_("Enter Name"), { 264, 211, 320, 33 }, UIS_CENTER | UIS_BIG),
...
UiItem SELLOAD_DIALOG[] = {
	UiArtText(N_("Save File Exists"), { 264, 211, 320, 33 }, UIS_CENTER | UIS_BIG),
	UiList(SELLOAD_DIALOG_ITEMS, 265, 285, 320, 33, UIS_CENTER | UIS_MED | UIS_GOLD),
	UiArtTextButton(N_("OK"), &UiFocusNavigationSelect, { 279, 427, 140, 35 }, UIS_CENTER | UIS_VCENTER | UIS_BIG | UIS_GOLD),
	UiArtTextButton(N_("Cancel"), &UiFocusNavigationEsc, { 429, 427, 140, 35 }, UIS_CENTER | UIS_VCENTER | UIS_BIG | UIS_GOLD)
};
...
void selhero_UpdateViewportItems()
{
	...
	if (num_viewport_heroes < kMaxViewportItems) {
		SELLIST_DIALOG_ITEMS[num_viewport_heroes] = { _("New Hero"), static_cast<int>(selhero_SaveCount) };
...
void selhero_List_Select(int value)
{
	if (static_cast<std::size_t>(value) == selhero_SaveCount) {
		...
		if (selhero_isMultiPlayer) {
			strcpy(title, _("New Multi Player Hero"));
		} else {
			strcpy(title, _("New Single Player Hero"));
		}
		return;
	...
	if (!UiValidPlayerName(selhero_heroInfo.name)) {
		...
		for (std::size_t i = 0; i < selhero_SaveCount; i++) {
			if (strcasecmp(selhero_heros[i].name, selhero_heroInfo.name) == 0) {
				...
				BlackPalette();
				char dialogText[256];
				sprintf(dialogText, _("Character already exists. Do you want to overwrite \"%s\"?"), selhero_heroInfo.name);
		...
		if (overwrite) {
			if (gfnHeroCreate(&selhero_heroInfo)) {
				...
				UiErrorOkDialog(_("Unable to create character."), SELHERO_DIALOG, size(SELHERO_DIALOG));
			}
		}
	}
    ...
    char *name)
{
	do {
		...
		if (selhero_navigateYesNo) {
			...
			sprintf(dialogText, _("Are you sure you want to delete the character \"%s\"?"), selhero_heroInfo.name);
