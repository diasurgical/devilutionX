...
bool protocol_zt::network_online()
{
	...
	if (fd_udp == -1) {
		...
		if (ret < 0) {
			LOG_ERROR(log::Network, "lwip, (udp) bind: {}", strerror(errno));
	...
	if (fd_tcp == -1) {
		...
		if (r1 < 0) {
			LOG_ERROR(log::Network, "lwip, (tcp) bind: {}", strerror(errno));
			throw protocol_exception();
		}
		auto r2 = lwip_listen(fd_tcp, 10);
		if (r2 < 0) {
			LOG_ERROR(log::Network, "lwip, listen: {}\n", strerror(errno));
			throw protocol_exception();
		}
		set_nonblock(fd_tcp);
		set_nodelay(fd_tcp);
...
bool protocol_zt::accept_all()
{
	...
	while (true) {
		...
		if (peer_list[ep].fd != -1) {
			LOG_WARN(log::Network, "protocol_zt::accept_all: WARNING: overwriting connection");
	...
	if (peer_list.count(peer)) {
		if (peer_list[peer].fd != -1) {
			if (lwip_close(peer_list[peer].fd) < 0) {
				LOG_WARN(log::Network, "lwip_close: {}", strerror(errno));
			}
		}
