...
eval cmake .. -DCMAKE_INSTALL_PREFIX=${SYSROOT} -DUNIX=1 ${DEFINES}
cmake --build . --config Release --target install -- -j$(nproc)
cd ../..
...
cd SDL_ttf-SDL-1.2
./autogen.sh
CFLAGS="${FLAGS}" CXXFLAGS="${FLAGS}" SDL_LIBS='-lSDL -ldebug' SDL_CFLAGS="-I${SYSROOT}/include/SDL -noixemul" FT2_CFLAGS="-I${SYSROOT}/include/freetype2" FT2_LIBS="-lfreetype -lzlib" ./configure --disable-shared --enable-static --host=${TARGET} --prefix=${SYSROOT}
make -j$(nproc)
make install
