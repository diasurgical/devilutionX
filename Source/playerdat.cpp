...
struct ExperienceColumnDefinition {
	...
	static ColumnType mapNameToType(string_view name)
	{
		if (name == "Level") {
...
{
	LoadDataResult dataFile = LoadDataFile("txtdata\\Experience.txt");
	...
	std::unordered_set<ExperienceColumnDefinition::ColumnType> seenColumns;
	...
	do {
		if (columns.size() >= ExpectedColumnCount) {
			...
			result = DiscardRemainingFields(result.ptr, dataFile.end());
			break;
		}

		std::string headerField;
		result = GetNextField(result.ptr, dataFile.end(), headerField);
		// As long as we don't call GetNextField twice it a row it will always contain a value
		ExperienceColumnDefinition::ColumnType columnType = ExperienceColumnDefinition::mapNameToType(headerField);
		if (columnType != ExperienceColumnDefinition::ColumnType::Unknown && seenColumns.find(columnType) == seenColumns.end()) {
			unsigned skipColumns = 0;
			if (currentColumn > lastColumn)
				skipColumns = currentColumn - lastColumn - 1;
			columns.emplace_back(columnType, skipColumns);
	...
	do {
		...
		for (auto &column : columns) {
			...
			case ExperienceColumnDefinition::ColumnType::Level: {
				auto fromCharsResult = from_chars(result.ptr, dataFile.end(), level);
				if (fromCharsResult.ec == std::errc::invalid_argument && !foundMaxLevelRecord) {
					...
					std::string buffer;
					result = GetNextField(fromCharsResult.ptr, dataFile.end(), buffer);
					if (buffer == "MaxLevel") {
