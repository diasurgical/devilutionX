...
bool AutoEquip(int playerNumber, const ItemStruct &item, int bodyLocation, bool persistItem)
{
	...
	if (persistItem) {
		...
		NetSendCmdChItem(true, bodyLocation);
...
void CheckInvPaste(int pnum, int mx, int my)
{
	...
	switch (il) {
	case ILOC_HELM:
		NetSendCmdChItem(true, INVLOC_HEAD);
	...
	case ILOC_RING:
		if (r == SLOTXY_RING_LEFT) {
			NetSendCmdChItem(true, INVLOC_RING_LEFT);
			...
			NetSendCmdChItem(true, INVLOC_RING_RIGHT);
		...
		break;
	case ILOC_AMULET:
		NetSendCmdChItem(true, INVLOC_AMULET);
	...
	case ILOC_TWOHAND:
		...
		break;
	case ILOC_ARMOR:
		NetSendCmdChItem(true, INVLOC_CHEST);
...
void CheckInvCut(int pnum, int mx, int my, bool automaticMove)
{
	...
	if (r == SLOTXY_RING_LEFT && !leftRingItem.isEmpty()) {
		...
		if (!automaticMove || automaticallyMoved) {
			NetSendCmdDelItem(true, INVLOC_RING_LEFT);
	...
	if (r == SLOTXY_RING_RIGHT && !rightRingItem.isEmpty()) {
		...
		if (!automaticMove || automaticallyMoved) {
			NetSendCmdDelItem(true, INVLOC_RING_RIGHT);
	...
	if (r == SLOTXY_AMULET && !amuletItem.isEmpty()) {
		...
		if (!automaticMove || automaticallyMoved) {
			NetSendCmdDelItem(true, INVLOC_AMULET);
...
void CleanupItems(ItemStruct *item, int ii)
{
	...
	if (currlevel == 21 & item->_ix == CornerStone.x && item->_iy == CornerStone.y) {
		...
		CornerStone.item._iAnimFlag = true;
		CornerStone.item._iIdentified = true;
		CornerStone.item._iPostDraw = true;
	}
