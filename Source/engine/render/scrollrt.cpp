...
void DrawDeadPlayer(const Surface &out, Point tilePosition, Point targetBufferPosition)
{
	...
	for (Player &player : Players) {
		if (player.plractive && player.life == 0 && player.isOnActiveLevel() && player.position.tile == tilePosition) {
			dFlags[tilePosition.x][tilePosition.y] |= DungeonFlag::DeadPlayer;
...
void DrawAndBlit()
{
	...
	if (*sgOptions.Gameplay.showHealthValues)
		DrawFlaskValues(out, { mainPanel.position.x + 134, mainPanel.position.y + 28 }, MyPlayer->life >> 6, MyPlayer->_pMaxHP >> 6);
