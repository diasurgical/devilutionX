...
void SyncPlrInv(TSyncHeader *pHdr)
{
	...
	if (ActiveItemCount > 0) {
		...
		pHdr->wItemIndx = static_cast<std::underlying_type_t<ItemID>>(SDL_SwapLE16(item.IDidx));
	...
	if (!item.isEmpty()) {
		...
		pHdr->wPInvIndx = static_cast<std::underlying_type_t<ItemID>>(SDL_SwapLE16(item.IDidx));
