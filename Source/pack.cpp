...
void VerifyGoldSeeds(Player &player)
{
	for (int i = 0; i < player._pNumInv; i++) {
		if (player.InvList[i].IDidx != ItemID::Gold)
			continue;
		for (int j = 0; j < player._pNumInv; j++) {
			if (i == j)
				continue;
			if (player.InvList[j].IDidx != ItemID::Gold)
				continue;
...
{
	packedItem.def.wIndx = static_cast<ItemID>(SDL_SwapLE16(item.IDidx));
...
{
	item = {};
	ItemID idx = SDL_SwapLE16(packedItem.def.wIndx);
	if (idx < ItemID::First || idx > ItemID::Last)
		return false;
	if (idx == ItemID::Ear) {
...
void PackItem(ItemPack &packedItem, const Item &item, bool isHellfire)
{
	...
	if (item.isEmpty() || item._iMiscId == IMISC_ARENAPOT) {
		...
		packedItem.idx = static_cast<std::underlying_type_t<ItemID>>(SDL_SwapLE16(idx));
		if (item.IDidx == ItemID::Ear) {
...
{
	auto idx = static_cast<ItemID>(SDL_SwapLE16(packedItem.idx));
