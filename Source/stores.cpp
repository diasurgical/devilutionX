...
void PrintStoreItem(ItemStruct *x, int l, text_color iclr)
{
	...
	if (sstr[0]) {
		AddSText(40, l, true, sstr, iclr, true);
		l++;
	}
	sstr[0] = '\0';
	...
	if (x->_iMagical == ITEM_QUALITY_UNIQUE) {
		if (x->_iIdentified)
			AddSText(40, l, true, "Unique Item", iclr, true);
	}
}
...
void S_StartSmith()
{
	stextsize = false;
	stextscrl = false;
	AddSText(0, 1, true, "Welcome to the", COL_GOLD, true);
	AddSText(0, 3, true, "Blacksmith's shop", COL_GOLD, true);
	AddSText(0, 7, true, "Would you like to:", COL_GOLD, true);
...
void S_ScrollSBuy(int idx)
{
	...
	for (l = 5; l < 20; l += 4) {
		if (!smithitem[ls].isEmpty()) {
			...
			if (smithitem[ls]._iMagical) {
				AddSText(20, l, true, smithitem[ls]._iIName, iclr, true);
			} else {
				AddSText(20, l, true, smithitem[ls]._iName, iclr, true);
			}
...
void S_StartSBuy()
{
	...
	AddSText(0, 1, true, tempstr, COL_GOLD, true);
	AddSLine(3);
	AddSLine(21);
	...
	for (l = 5; l < 20 && idx < SMITH_PREMIUM_ITEMS; l += 4) {
		if (!premiumitem[idx].isEmpty()) {
			...
			if (!premiumitem[idx]._iStatFlag)
				iclr = COL_RED;
			AddSText(20, l, true, premiumitem[idx]._iIName, iclr, true);
	...
	for (l = 5; l < 20; l += 4) {
		...
		if (!storehold[idx].isEmpty()) {
			...
			if (storehold[idx]._iMagical && storehold[idx]._iIdentified) {
				AddSText(20, l, true, storehold[idx]._iIName, iclr, true);
				AddSTextVal(l, storehold[idx]._iIvalue);
			} else {
				AddSText(20, l, true, storehold[idx]._iName, iclr, true);
...
void S_StartWitch()
{
	...
	stextsize = false;
	stextscrl = false;
	AddSText(0, 2, true, "Witch's shack", COL_GOLD, true);
	AddSText(0, 9, true, "Would you like to:", COL_GOLD, true);
...
void S_ScrollWBuy(int idx)
{
	...
	for (l = 5; l < 20; l += 4) {
		if (!witchitem[ls].isEmpty()) {
			...
			if (witchitem[ls]._iMagical) {
				AddSText(20, l, true, witchitem[ls]._iIName, iclr, true);
			} else {
				AddSText(20, l, true, witchitem[ls]._iName, iclr, true);
			}
...
void S_StartConfirm()
{
	...
	if (idprint)
		AddSText(20, 8, true, plr[myplr].HoldItem._iIName, iclr, true);
	else
		AddSText(20, 8, true, plr[myplr].HoldItem._iName, iclr, true);
	...
	AddSText(0, 15, true, tempstr, COL_WHITE, true);
...
void S_StartBoy()
{
	stextsize = false;
	stextscrl = false;
	AddSText(0, 2, true, "Wirt the Peg-legged boy", COL_GOLD, true);
	AddSLine(5);
	if (!boyitem.isEmpty()) {
		AddSText(0, 8, true, "Talk to Wirt", COL_BLUE, true);
		AddSText(0, 12, true, "I have something for sale,", COL_GOLD, true);
		AddSText(0, 14, true, "but it will cost 50 gold", COL_GOLD, true);
		AddSText(0, 16, true, "just to take a look. ", COL_GOLD, true);
...
void S_StartBBoy()
{
	...
	if (!boyitem._iStatFlag)
		iclr = COL_RED;
	if (boyitem._iMagical != ITEM_QUALITY_NORMAL)
		AddSText(20, 10, true, boyitem._iIName, iclr, true);
	else
		AddSText(20, 10, true, boyitem._iName, iclr, true);
...
void S_StartHealer()
{
	...
	stextscrl = false;
	AddSText(0, 1, true, "Welcome to the", COL_GOLD, true);
	AddSText(0, 3, true, "Healer's home", COL_GOLD, true);
	AddSText(0, 9, true, "Would you like to:", COL_GOLD, true);
...
void S_ScrollHBuy(int idx)
{
	...
	for (l = 5; l < 20; l += 4) {
		if (!healitem[idx].isEmpty()) {
			...
			AddSText(20, l, true, healitem[idx]._iName, iclr, true);
...
void S_StartStory()
{
	stextsize = false;
	stextscrl = false;
	AddSText(0, 2, true, "The Town Elder", COL_GOLD, true);
	AddSText(0, 9, true, "Would you like to:", COL_GOLD, true);
...
void S_StartIdShow()
{
	...
	AddSText(0, 7, true, "This item is:", COL_WHITE, true);
	AddSText(20, 11, true, plr[myplr].HoldItem._iIName, iclr, true);
...
void S_StartTalk()
{
	...
	AddSText(0, 2, true, tempstr, COL_GOLD, true);
	AddSLine(5);
	if (gbIsSpawn) {
		sprintf(tempstr, "Talking to %s", talkname[talker]);
		AddSText(0, 10, true, tempstr, COL_WHITE, true);
		AddSText(0, 12, true, "is not available", COL_WHITE, true);
		AddSText(0, 14, true, "in the shareware", COL_WHITE, true);
		AddSText(0, 16, true, "version", COL_WHITE, true);
		AddSText(0, 22, true, "Back", COL_WHITE, true);
		return;
	}
...
void S_StartTavern()
{
	...
	stextscrl = false;
	AddSText(0, 1, true, "Welcome to the", COL_GOLD, true);
	AddSText(0, 3, true, "Rising Sun", COL_GOLD, true);
	AddSText(0, 9, true, "Would you like to:", COL_GOLD, true);
...
void S_StartBarMaid()
{
	stextsize = false;
	stextscrl = false;
	AddSText(0, 2, true, "Gillian", COL_GOLD, true);
	AddSText(0, 9, true, "Would you like to:", COL_GOLD, true);
...
void S_StartDrunk()
{
	stextsize = false;
	stextscrl = false;
	AddSText(0, 2, true, "Farnham the Drunk", COL_GOLD, true);
	AddSText(0, 9, true, "Would you like to:", COL_GOLD, true);
