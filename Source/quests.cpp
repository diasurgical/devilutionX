...
void CheckQuestKill(int m, bool sendmsg)
{
	...
	if (monster[m].MType->mtype == MT_SKING) {
		...
		sfxdelay = 30;
		sfxdnum = GetSoundByClass(myplr, 82);
		if (sendmsg)
		...
		sfxdnum = GetSoundByClass(myplr, 83);
		if (sendmsg) {
			NetSendCmdQuest(true, Q_BETRAYER);
		...
		sfxdelay = 30;
		sfxdnum = GetSoundByClass(myplr, 94);
	}
}
