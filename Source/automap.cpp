...
WORD GetAutomapType(int x, int y, bool view)
{
	...
	if (view && x == -1 && y >= 0 && y < DMAXY && automapview[0][y]) {
		if (GetAutomapType(0, y, true) & (MAPFLAG_DIRT << 8)) {
			return 0;
		} else {
	...
	if (view && y == -1 && x >= 0 && x < DMAXY && automapview[x][0]) {
		if (GetAutomapType(x, 0, true) & (MAPFLAG_DIRT << 8)) {
			return 0;
		} else {
			return MAPFLAG_DIRT << 8;
		}
	...
	if (rv == 7) {
		if ((GetAutomapType(x - 1, y, true) >> 8) & MAPFLAG_HORZARCH) {
			if ((GetAutomapType(x, y - 1, true) >> 8) & MAPFLAG_VERTARCH) {
				rv = 1;
			}
		}
	}
...
void SetAutomapView(Sint32 x, Sint32 y)
{
	...
	maptype = GetAutomapType(xx, yy, true);
	solid = maptype & 0x4000;

	switch (maptype & MAPFLAG_TYPE) {
	case 2:
		if (solid) {
			if (GetAutomapType(xx, yy + 1, true) == 0x4007)
				automapview[xx][yy + 1] = true;
		} else if (GetAutomapType(xx - 1, yy, true) & 0x4000) {
			automapview[xx - 1][yy] = true;
		}
		break;
	case 3:
		if (solid) {
			if (GetAutomapType(xx + 1, yy, true) == 0x4007)
				automapview[xx + 1][yy] = true;
		} else if (GetAutomapType(xx, yy - 1, true) & 0x4000) {
			automapview[xx][yy - 1] = true;
		}
		break;
	case 4:
		if (solid) {
			...
			if (GetAutomapType(xx - 1, yy, true) & 0x4000)
				automapview[xx - 1][yy] = true;
			if (GetAutomapType(xx, yy - 1, true) & 0x4000)
				automapview[xx][yy - 1] = true;
			if (GetAutomapType(xx - 1, yy - 1, true) & 0x4000)
