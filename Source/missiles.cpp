...
void GetMissilePos(int i)
{
	...
	mx = missile[i]._mitxoff / 65536;
	my = missile[i]._mityoff / 65536;
	dx = mx + 2 * my;
	dy = 2 * my - mx;
	if (dx < 0) {
		lx = -(-dx / 8);
...
#endif
		if (t == MIS_BONESPIRIT) {
			dam = monster[m]._mhitpoints / 3 / 64;
		} else {
...
void AddCbolt(Sint32 mi, Sint32 sx, Sint32 sy, Sint32 dx, Sint32 dy, Sint32 midir, Sint8 micaster, Sint32 id, Sint32 dam)
{
	...
	if (micaster == 0) {
		if (id == myplr) {
			missile[mi]._mirnd = random_(63, 15) + 1;
			missile[mi]._midam = random_(68, plr[id]._pMagic / 4) + 1;
		} else {
			missile[mi]._mirnd = random_(63, 15) + 1;
			missile[mi]._midam = random_(68, plr[id]._pMagic / 4) + 1;
		}
...
int Sentfire(int i, int sx, int sy)
{
	...
	if (LineClear(missile[i]._mix, missile[i]._miy, sx, sy)) {
		if (dMonster[sx][sy] > 0 && monster[dMonster[sx][sy] - 1]._mhitpoints / 64 > 0 && dMonster[sx][sy] - 1 > MAX_PLRS - 1) {
	...
	if (missile[i]._mitype != MIS_BONESPIRIT || missile[i]._mimfnum != 8) {
		...
		if (p != -1) {
			if (missile[i]._micaster == TARGET_MONSTERS) {
				...
				case MIS_FIREBOLT:
					d = random_(75, 10) + (plr[p]._pMagic / 8) + missile[i]._mispllvl + 1;
					break;
				case MIS_FLARE:
	...
	if (id != myplr) {
		...
		if (plr[id]._pHitPoints < missile[i]._miVar1) {
			...
			if (plr[id]._pMana >= diff) {
				...
				if ((plr[id]._pHitPoints / 64) == 0 && id == myplr) {
...
void MI_Bonespirit(Sint32 i)
{
	...
	if (missile[i]._mimfnum == 8) {
		...
		if (missile[i]._miVar3 == 1) {
			...
			mid = FindClosest(cx, cy, 19);
			if (mid > 0) {
				missile[i]._midam = monster[mid]._mhitpoints / 128;
