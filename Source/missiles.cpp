...
{
	for (int i = 0; i < 3; i++) {
		for (DisplacementOf<int8_t> displacement : CrawlTableRow(i)) {
...
void MI_FireRing(Missile &missile)
{
	...
	for (DisplacementOf<int8_t> displacement : CrawlTableRow(3)) {
...
void MI_Chain(Missile &missile)
{
	...
	int rad = std::min<int>(missile._mispllvl + 3, CrawlTableRows - 1);
	for (int i = 1; i < rad; i++) {
		for (DisplacementOf<int8_t> displacement : CrawlTableRow(i)) {
