...
bool left_shoulder_modifier_active = false;
...
bool GetGameAction(const SDL_Event &event, ControllerButtonEvent ctrlEvent, GameAction *action) {
	if (!sgOptions.Controller.bSimplifiedScheme) {
		return GetStandardGameAction(event, ctrlEvent, action);
	} else {
		return GetSimplifiedGameAction(event, ctrlEvent, action);
	}
}
	...
	if (!inGameMenu) {
		switch (ctrlEvent.button) {
			...
			case ControllerButton_BUTTON_X:
				...
				break;
			case ControllerButton_BUTTON_LEFTSHOULDER:
				left_shoulder_modifier_active = !ctrlEvent.up;
				return true;
			case ControllerButton_BUTTON_RIGHTSHOULDER:
				gbLockPlayerPosition = !ctrlEvent.up;
				return true;
			case ControllerButton_BUTTON_START:
				if (!ctrlEvent.up) {
					start_modifier_active = !start_modifier_active;
				}
				return true;
			case ControllerButton_IGNORE:
			case ControllerButton_BUTTON_BACK:
		...
		// Handle Belt
		if (left_shoulder_modifier_active) {
			switch (ctrlEvent.button) {
				case ControllerButton_BUTTON_A:
					*action = GameAction(GameActionType_USE_HEALTH_POTION);
					return true;
