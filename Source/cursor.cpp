	...
	if (leveltype != DTYPE_TOWN) {
		if (pcurstemp != -1) {
			if (!flipflag && mx + 2 < MAXDUNX && my + 1 < MAXDUNY && dMonster[mx + 2][my + 1] != 0 && IsTileLit({ mx + 2, my + 1 })) {
				...
				if (mi == pcurstemp && Monsters[mi].hitPoints >> 6 > 0 && (Monsters[mi].data().sellFlag & 4) != 0) {
			...
			if (mx + 1 < MAXDUNX && !flipflag && dMonster[mx + 1][my] != 0 && IsTileLit({ mx + 1, my })) {
				...
				if (mi == pcurstemp && Monsters[mi].hitPoints >> 6 > 0 && (Monsters[mi].data().sellFlag & 2) != 0) {
			...
			if (dMonster[mx][my] != 0 && IsTileLit({ mx, my })) {
				...
				if (mi == pcurstemp && Monsters[mi].hitPoints >> 6 > 0 && (Monsters[mi].data().sellFlag & 1) != 0) {
					cursPosition = { mx, my };
					pcursmonst = mi;
				}
			}
		...
		if (!flipflag && mx + 2 < MAXDUNX && my + 1 < MAXDUNY && dMonster[mx + 2][my + 1] != 0 && IsTileLit({ mx + 2, my + 1 })) {
			int mi = abs(dMonster[mx + 2][my + 1]) - 1;
			if (Monsters[mi].hitPoints >> 6 > 0 && (Monsters[mi].data().sellFlag & 4) != 0) {
				cursPosition = Point { mx, my } + Displacement { 2, 1 };
				pcursmonst = mi;
			}
		}
		...
		if (!flipflag && mx + 1 < MAXDUNX && dMonster[mx + 1][my] != 0 && IsTileLit({ mx + 1, my })) {
			int mi = abs(dMonster[mx + 1][my]) - 1;
			if (Monsters[mi].hitPoints >> 6 > 0 && (Monsters[mi].data().sellFlag & 2) != 0) {
				cursPosition = Point { mx, my } + Displacement { 1, 0 };
				pcursmonst = mi;
			}
		}
		...
		if (dMonster[mx][my] != 0 && IsTileLit({ mx, my })) {
			int mi = abs(dMonster[mx][my]) - 1;
			if (Monsters[mi].hitPoints >> 6 > 0 && (Monsters[mi].data().sellFlag & 1) != 0) {
				cursPosition = { mx, my };
				pcursmonst = mi;
			}
		}
