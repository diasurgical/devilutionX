...
void CreatePlayer(int pnum, plr_class c)
{
	...
	for (i = 0; i < NUMLEVELS; i++) {
		plr[pnum]._pLvlVisited[i] = true;
	}

	for (i = 0; i < 10; i++) {
		plr[pnum]._pSLvlVisited[i] = true;
	}

	plr[pnum]._pLvlChanging = true;
	plr[pnum].pTownWarps = 0;
	plr[pnum].pLvlLoad = 0;
	plr[pnum].pBattleNet = true;
...
void AddPlrExperience(int pnum, int lvl, int exp)
{
	...
	NetSendCmdParam1(true, CMD_PLRLEVEL, plr[myplr]._pLevel);
}
...
static void PlrDeadItem(int pnum, ItemStruct *itm, int xx, int yy)
{
	...
	if ((xx || yy) && ItemSpaceOk(x, y)) {
		...
		NetSendCmdPItem(true, CMD_RESPAWNITEM, x, y);
		return;
	}
...
void CheckNewPath(int pnum)
{
	...
	if (plr[pnum].destAction == ACTION_ATTACKMON) {
		...
		MakePlrPath(pnum, monster[i]._mfutx, monster[i]._mfuty, true);
	}

	if (plr[pnum].destAction == ACTION_ATTACKPLR) {
		i = plr[pnum].destParam1;
		MakePlrPath(pnum, plr[i]._pfutx, plr[i]._pfuty, true);
	}
...
void CheckPlrSpell()
{
	...
	case RSPLTYPE_SKILL:
	case RSPLTYPE_SPELL:
		addflag = CheckSpell(myplr, rspell, plr[myplr]._pRSplType, true);
		break;
...
void ModifyPlrStr(int p, int l)
{
	...
	if (p == myplr) {
		NetSendCmdParam1(true, CMD_SETSTR, plr[p]._pBaseStr);
	}
}
...
void ModifyPlrMag(int p, int l)
{
	...
	if (p == myplr) {
		NetSendCmdParam1(true, CMD_SETMAG, plr[p]._pBaseMag);
	}
}
...
void ModifyPlrDex(int p, int l)
{
	...
	if (p == myplr) {
		NetSendCmdParam1(true, CMD_SETDEX, plr[p]._pBaseDex);
	}
}
...
void ModifyPlrVit(int p, int l)
{
	...
	if (p == myplr) {
		NetSendCmdParam1(true, CMD_SETVIT, plr[p]._pBaseVit);
	}
}
