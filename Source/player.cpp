...
bool PlrHitMonst(int pnum, int m)
{
	...
	if (hit < hper) {
		...
		if (player._pIFlags & ISPL_FIREDAM && player._pIFlags & ISPL_LIGHTDAM) {
			int midam = player._pIFMinDam + GenerateRnd_rngv2(player._pIFMaxDam - player._pIFMinDam);
		...
		dam = GenerateRnd_rngv2(maxd - mind + 1) + mind;
		...
		if (player.pDamAcFlags & 0x01 && GenerateRnd_rngv2(100) < 5) {
			dam *= 3;
		}

		if (player.pDamAcFlags & 0x10 && monster[m].MType->mtype != MT_DIABLO && monster[m]._uniqtype == 0 && GenerateRnd_rngv2(100) < 10) {
			monster_43C785(m);
		}

		dam <<= 6;
		if ((player.pDamAcFlags & 0x08) != 0) {
			int r = GenerateRnd_rngv2(201);
			if (r >= 100)
				r = 100 + (r - 100) * 5;
			dam = dam * r / 100;
		}
	...
	if (hit < hper) {
		if (blk < blkper) {
			...
			if (attacker._pClass == HeroClass::Warrior || attacker._pClass == HeroClass::Barbarian) {
				...
				if (GenerateRnd_rngv2(100) < lvl) {
					dam *= 2;
				}
			}
			skdam = dam << 6;
			if ((attacker._pIFlags & ISPL_RNDSTEALLIFE) != 0) {
				tac = GenerateRnd_rngv2(skdam / 8);
...
bool PM_DoRangeAttack(int pnum)
{
	...
	for (int arrow = 0; arrow < arrows; arrow++) {
		...
		if ((player._pIFlags & ISPL_FIRE_ARROWS) != 0 && (player._pIFlags & ISPL_LIGHT_ARROWS) != 0) {
			dmg = player._pIFMinDam + GenerateRnd_rngv2(player._pIFMaxDam - player._pIFMinDam);
