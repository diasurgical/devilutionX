...
void CheckPlrSpell()
{
	...
	if (!sgbControllerActive) {
		...
		if (rectMainPanel.Contains(MousePosition)) // inside main panel
			return;

		if (
		    ((chrflag || QuestLogIsOpen) && rectLeftPanel.Contains(MousePosition)) // inside left panel
		    || ((invflag || sbookflag) && rectRightPanel.Contains(MousePosition)) // inside right panel
		) {
			if (rspell != SPL_HEAL
			    && rspell != SPL_IDENTIFY
			    && rspell != SPL_REPAIR
