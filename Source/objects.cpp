...
void InitObjectGFX()
{
	...
	memset(fileload, true, sizeof(fileload));
...
void Obj_FlameTrap(int i)
{
	...
	if (object[i]._oVar2 != 0) {
		...
		y = object[i]._oy;
		if (dMonster[x][y] > 0)
			MonsterTrapHit(dMonster[x][y] - 1, mindam / 2, maxdam / 2, 0, MIS_FIREWALLC, true);
		if (dPlayer[x][y] > 0) {
			bool unused;
			PlayerMHit(dPlayer[x][y] - 1, -1, 0, mindam, maxdam, MIS_FIREWALLC, true, 0, &unused);
		}
...
void OperateLever(int pnum, int i)
{
	...
	if (object[i]._oSelFlag != 0) {
		...
		if (pnum == myplr)
			NetSendCmdParam1(true, CMD_OPERATEOBJ, i);
	}
}
		...
		if (!deltaload) {
			...
			if (setlevel) {
				for (j = 0; j < object[i]._oVar1; j++) {
					CreateRndItem(object[i]._ox, object[i]._oy, true, sendmsg, true);
				}
			} else {
				for (j = 0; j < object[i]._oVar1; j++) {
					if (object[i]._oVar2 != 0)
			...
			if (pnum == myplr)
				NetSendCmdParam2(true, CMD_PLROPOBJ, pnum, i);
			return;
		}
	}
}
	...
	if (object[i]._oSelFlag != 0) {
		...
		if (!deltaload) {
			if (plr[pnum]._pClass == PC_WARRIOR) {
				CreateMagicArmor(object[i]._ox, object[i]._oy, ITYPE_HARMOR, ICURS_BREAST_PLATE, true, true);
				PlaySfxLoc(PS_WARR9, plr[myplr]._px, plr[myplr]._py);
			} else if (plr[pnum]._pClass == PC_ROGUE) {
				CreateMagicWeapon(object[i]._ox, object[i]._oy, ITYPE_BOW, ICURS_LONG_WAR_BOW, true, true);
				PlaySfxLoc(PS_ROGUE9, plr[myplr]._px, plr[myplr]._py);
			} else if (plr[pnum]._pClass == PC_SORCERER) {
				CreateSpellBook(object[i]._ox, object[i]._oy, SPL_LIGHTNING, true, true);
				PlaySfxLoc(PS_MAGE9, plr[myplr]._px, plr[myplr]._py);
			} else if (plr[pnum]._pClass == PC_MONK) {
				CreateMagicWeapon(object[i]._ox, object[i]._oy, ITYPE_STAFF, ICURS_WAR_STAFF, true, true);
				PlaySfxLoc(PS_MONK9, plr[myplr]._px, plr[myplr]._py);
			} else if (plr[pnum]._pClass == PC_BARD) {
				CreateMagicWeapon(object[i]._ox, object[i]._oy, ITYPE_SWORD, ICURS_BASTARD_SWORD, true, true);
				PlaySfxLoc(PS_ROGUE9, plr[myplr]._px, plr[myplr]._py);
			} else if (plr[pnum]._pClass == PC_BARBARIAN) {
				CreateMagicWeapon(object[i]._ox, object[i]._oy, ITYPE_AXE, ICURS_BATTLE_AXE, true, true);
				PlaySfxLoc(PS_WARR9, plr[myplr]._px, plr[myplr]._py);
			}
			if (pnum == myplr)
				NetSendCmdParam1(true, CMD_OPERATEOBJ, i);
...
void OperateShrine(int pnum, int i, _sfx_id sType)
{
	...
	case SHRINE_DIVINE:
		...
		if (pnum != myplr)
			return;
		if (2 * currlevel < 7) {
			CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_MISC, IMISC_FULLMANA, true, true);
			CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_MISC, IMISC_FULLHEAL, true, true);
		} else {
			CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_MISC, IMISC_FULLREJUV, true, true);
			CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_MISC, IMISC_FULLREJUV, true, true);
		}
		plr[pnum]._pMana = plr[pnum]._pMaxMana;
		plr[pnum]._pManaBase = plr[pnum]._pMaxManaBase;
...
void OperateSkelBook(int pnum, int i, bool sendmsg)
{
	if (object[i]._oSelFlag != 0) {
		...
		if (!deltaload) {
			SetRndSeed(object[i]._oRndSeed);
			if (random_(161, 5) != 0)
				CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_MISC, IMISC_SCROLL, sendmsg, true);
			else
				CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_MISC, IMISC_BOOK, sendmsg, true);
			if (pnum == myplr)
				NetSendCmdParam1(true, CMD_OPERATEOBJ, i);
		}
	}
...
void OperateArmorStand(int pnum, int i, bool sendmsg)
{
	...
	if (object[i]._oSelFlag != 0) {
		...
		if (!deltaload) {
			...
			if (currlevel <= 5) {
				CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_LARMOR, IMISC_NONE, sendmsg, true);
			} else if (currlevel >= 6 && currlevel <= 9) {
				CreateTypeItem(object[i]._ox, object[i]._oy, uniqueRnd, ITYPE_MARMOR, IMISC_NONE, sendmsg, true);
			} else if (currlevel >= 10 && currlevel <= 12) {
				CreateTypeItem(object[i]._ox, object[i]._oy, true, ITYPE_HARMOR, IMISC_NONE, sendmsg, true);
...
void OperateWeaponRack(int pnum, int i, bool sendmsg)
{
	...
	CreateTypeItem(object[i]._ox, object[i]._oy, leveltype > 1, weaponType, IMISC_NONE, sendmsg, true);
...
void SyncOpL1Door(int pnum, int cmd, int i)
{
	...
	if (do_sync) {
		if (object[i]._otype == OBJ_L1LDOOR)
			OperateL1LDoor(-1, i, true);
		if (object[i]._otype == OBJ_L1RDOOR)
			OperateL1RDoor(-1, i, true);
	}
}
...
void SyncOpL2Door(int pnum, int cmd, int i)
{
	...
	if (do_sync) {
		if (object[i]._otype == OBJ_L2LDOOR)
			OperateL2LDoor(-1, i, true);
		if (object[i]._otype == OBJ_L2RDOOR)
			OperateL2RDoor(-1, i, true);
	}
}
...
void SyncOpL3Door(int pnum, int cmd, int i)
{
	...
	if (do_sync) {
		if (object[i]._otype == OBJ_L3LDOOR)
			OperateL3LDoor(-1, i, true);
		if (object[i]._otype == OBJ_L3RDOOR)
			OperateL3RDoor(-1, i, true);
	}
}
...
void SyncOpObject(int pnum, int cmd, int i)
{
	...
	case OBJ_TCHEST2:
	case OBJ_TCHEST3:
		OperateChest(pnum, i, true);
		break;
	case OBJ_SARC:
		OperateSarc(pnum, i, true);
		break;
	case OBJ_BLINDBOOK:
	case OBJ_BLOODBOOK:
	case OBJ_STEELTOME:
	...
	case OBJ_SKELBOOK:
	case OBJ_BOOKSTAND:
		OperateSkelBook(pnum, i, true);
		break;
	case OBJ_BOOKCASEL:
	case OBJ_BOOKCASER:
		OperateBookCase(pnum, i, true);
		break;
	case OBJ_DECAP:
		OperateDecap(pnum, i, true);
		break;
	case OBJ_ARMORSTAND:
	case OBJ_WARARMOR:
		OperateArmorStand(pnum, i, true);
		break;
	case OBJ_GOATSHRINE:
		OperateGoatShrine(pnum, i, LS_GSHRINE);
		break;
	...
	case OBJ_WARWEAP:
	case OBJ_WEAPONRACK:
		OperateWeaponRack(pnum, i, true);
		break;
	case OBJ_MUSHPATCH:
		OperateMushPatch(pnum, i);
		break;
	case OBJ_SLAINHERO:
		OperateSlainHero(pnum, i, true);
		break;
	case OBJ_SIGNCHEST:
		OperateInnSignChest(pnum, i);
		break;
...
void BreakBarrel(int pnum, int i, int dam, bool forcebreak, bool sendmsg)
{
	...
	if (object[i]._otype == OBJ_BARRELEX) {
		...
		for (yp = object[i]._oy - 1; yp <= object[i]._oy + 1; yp++) {
			for (xp = object[i]._ox - 1; xp <= object[i]._ox + 1; xp++) {
				if (dMonster[xp][yp] > 0)
					MonsterTrapHit(dMonster[xp][yp] - 1, 1, 4, 0, MIS_FIREBOLT, true);
				bool unused;
				if (dPlayer[xp][yp] > 0)
					PlayerMHit(dPlayer[xp][yp] - 1, -1, 0, 8, 16, MIS_FIREBOLT, true, 0, &unused);
				if (dObject[xp][yp] > 0) {
					oi = dObject[xp][yp] - 1;
	...
	if (pnum == myplr)
		NetSendCmdParam2(true, CMD_BREAKOBJ, pnum, i);
}
...
void BreakObject(int pnum, int oi)
{
	...
	case OBJ_BARREL:
	case OBJ_BARRELEX:
		BreakBarrel(pnum, oi, objdam, true, true);
		break;
	}
}
...
void SyncBreakObj(int pnum, int oi)
{
	if (object[oi]._otype >= OBJ_BARREL && object[oi]._otype <= OBJ_BARRELEX)
		BreakBarrel(pnum, oi, 0, true, true);
}
