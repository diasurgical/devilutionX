...
void OperateBookLever(Object &questBook, bool sendmsg)
{
	...
	if (questBook._oSelFlag != 0 && !qtextflag) {
		...
		if (questBook._otype == OBJ_BLOODBOOK && Quests[Q_BLOOD]._qvar1 == 0) {
			...
			if (sendmsg)
				SpawnQuestItem(ItemID::BloodStone, SetPiece.position.megaToWorld() + Displacement { 9, 17 }, 0, 1, true);
		}
...
void OperateMushroomPatch(const Player &player, Object &mushroomPatch)
{
	...
	if (&player == MyPlayer) {
		SpawnQuestItem(ItemID::BlackMushroom, pos, 0, 0, true);
...
void OperateInnSignChest(const Player &player, Object &questContainer, bool sendmsg)
{
	...
	if (sendmsg) {
		...
		SpawnQuestItem(ItemID::TavernSign, pos, 0, 0, true);
...
void OperatePedestal(Player &player, Object &pedestal, bool sendmsg)
{
	...
	if (pedestal._oVar6 == 3 || (sendmsg && !RemoveInventoryItemById(player, ItemID::BloodStone))) {
		return;
	}
	...
	if (pedestal._oVar6 == 1) {
		...
		if (sendmsg)
			SpawnQuestItem(ItemID::BloodStone, SetPiece.position.megaToWorld() + Displacement { 3, 10 }, 0, 1, true);
	}
	if (pedestal._oVar6 == 2) {
		...
		if (sendmsg)
			SpawnQuestItem(ItemID::BloodStone, SetPiece.position.megaToWorld() + Displacement { 15, 10 }, 0, 1, true);
	}
...
void OperateLazStand(Object &stand)
{
	...
	SpawnQuestItem(ItemID::StaffOfLazarus, pos, 0, 0, true);
