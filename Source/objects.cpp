...
void TryDisarm(int pnum, int i)
{
	...
	if (object[i]._oTrapFlag) {
		...
		if (GenerateRndV2(100) <= trapdisper) {
			for (j = 0; j < nobjects; j++) {
				checkflag = false;
				oi = objectactive[j];
...
bool OperateShrineMysterious(int pnum)
{
	...
	switch (static_cast<CharacterAttribute>(GenerateRndV2(4))) {
...
bool OperateShrineHidden(int pnum)
{
	...
	if (cnt > 0) {
		...
		while (true) {
			...
			if (cnt == 0)
				break;
			int r = GenerateRndV2(NUM_INVLOC);
...
bool OperateShrineHoly(int pnum)
{
	...
	DWORD lv;
	do {
		xx = GenerateRndV2(MAXDUNX);
		yy = GenerateRndV2(MAXDUNY);
		lv = dPiece[xx][yy];
		j++;
...
bool OperateShrineSpiritual(int pnum)
{
	...
	for (int8_t &gridItem : plr[pnum].InvGrid) {
		if (gridItem == 0) {
			int r = 5 * leveltype + GenerateRndV2(10 * leveltype);
...
bool OperateShrineMurphys(int pnum)
{
	...
	for (auto &item : plr[myplr].InvBody) {
		if (!item.isEmpty() && GenerateRndV2(3) == 0) {
...
void BreakObject(int pnum, int oi)
{
	...
	if (pnum != -1) {
		...
		objdam = GenerateRndV2(maxd - mind + 1) + mind;
