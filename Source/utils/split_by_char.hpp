...
#include "stdcompat/string_view.hpp"
...
public:
	...
	static SplitByCharIterator begin(string_view text, char split_by)
	{
		return SplitByCharIterator(split_by, text, text.substr(0, text.find(split_by)));
	}

	static SplitByCharIterator end(string_view text, char split_by)
	{
