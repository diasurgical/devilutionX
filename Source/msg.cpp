...
bool DeltaGetItem(const TCmdGItem &message, uint8_t bLevel)
{
	...
	for (TCmdPItem &delta : deltaLevel.item) {
		if (delta.bCmd == CMD_INVALID) {
			...
			if (message.def.wIndx == ItemID::Ear) {
...
bool IsGItemValid(const TCmdGItem &message)
{
	...
	return IsItemAvailable(GetItemIDIndex(SDL_SwapLE16(message.def.wIndx)));
}
...
{
	message.def.wIndx = static_cast<ItemID>(SDL_SwapLE16(item.IDidx));
...
{
	if (message.def.wIndx == SDL_SwapLE16(ItemID::Ear))
...
int SyncDropItem(Point position, const TItem &item)
{
	return SyncDropItem(
	    position,
	    static_cast<ItemID>(SDL_SwapLE16(item.wIndx)),
...
size_t OnRequestGetItem(const TCmd *pCmd, Player &player)
{
	...
	if (gbBufferMsgs != 1 && IOwnLevel(player) && IsGItemValid(message)) {
		...
		const ItemID wIndx = static_cast<ItemID>(SDL_SwapLE16(message.def.wIndx));
		if (GetItemRecord(dwSeed, wCI, GetItemIDIndex(wIndx))) {
			int ii = -1;
			if (InDungeonBounds(position)) {
			...
			if (ii != -1) {
				...
				SetItemRecord(dwSeed, wCI, GetItemIDIndex(wIndx));
		...
		if (DeltaGetItem(message, message.bLevel)) {
			...
			if ((localLevel == message.bLevel || message.bPnum == MyPlayerId) && message.bMaster != MyPlayerId) {
				if (message.bPnum == MyPlayerId) {
					...
					SyncGetItem(position, SDL_SwapLE32(message.def.dwSeed), static_cast<ItemID>(SDL_SwapLE16(message.def.wIndx)), SDL_SwapLE16(message.def.wCI));
				}
			}
		} else {
...
size_t OnPutItem(const TCmd *pCmd, size_t pnum)
{
	...
	if (gbBufferMsgs == 1) {
		...
		if (player.isOnActiveLevel()) {
			...
			if (ii != -1) {
				PutItemRecord(dwSeed, wCI, GetItemIDIndex(wIndx));
...
size_t OnChangePlayerItems(const TCmd *pCmd, size_t pnum)
{
	...
	if (gbBufferMsgs == 1) {
		SendPacket(pnum, &message, sizeof(message));
	} else if (&player != MyPlayer && IsItemAvailable(GetItemIDIndex(SDL_SwapLE16(message.def.wIndx)))) {
...
size_t OnSpawnItem(const TCmd *pCmd, size_t pnum)
{
	...
	if (gbBufferMsgs == 1) {
		...
		PutItemRecord(SDL_SwapLE32(message.def.dwSeed), SDL_SwapLE16(message.def.wCI), GetItemIDIndex(SDL_SwapLE16(message.def.wIndx)));
		DeltaPutItem(message, { message.x, message.y }, player);
	}
...
void RecreateItem(const Player &player, const TItem &messageItem, Item &item)
{
	...
	RecreateItem(player, item,
	    static_cast<ItemID>(SDL_SwapLE16(messageItem.wIndx)), SDL_SwapLE16(messageItem.wCI),
...
void DeltaAddItem(int ii)
{
	...
	for (const TCmdPItem &item : deltaLevel.item) {
		if (item.bCmd != CMD_INVALID
		    && static_cast<ItemID>(SDL_SwapLE16(item.def.wIndx)) == Items[ii].IDidx
	...
	for (int i = 0; i < MAXITEMS; i++) {
		...
		if (deltaLevel.item[i].bCmd == TCmdPItem::PickedUpItem) {
			int activeItemIndex = FindGetItem(
			    ...
			    static_cast<ItemID>(SDL_SwapLE16(deltaLevel.item[i].def.wIndx)),
