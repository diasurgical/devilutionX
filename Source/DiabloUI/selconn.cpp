...
void SelconnLoad()
{
	...
	vecConnItems.push_back(new UiListItem(_(/* UI Element */ "Client-Server (TCP)"), SELCONN_TCP));
#endif
	vecConnItems.push_back(new UiListItem(_(/* UI Element */ "Loopback"), SELCONN_LOOPBACK));
	...
	vecSelConnDlg.push_back(new UiArtText(_(/* UI Element */ "Multi Player Game"), rect1, UIS_CENTER | UIS_BIG));
	...
	vecSelConnDlg.push_back(new UiArtText(_(/* UI Element */ "Requirements:"), rect3));
	...
	vecSelConnDlg.push_back(new UiArtText(_(/* UI Element */ "no gateway needed"), rect5, UIS_CENTER | UIS_MED));
	...
	vecSelConnDlg.push_back(new UiArtText(_(/* UI Element */ "Select Connection"), rect7, UIS_CENTER | UIS_BIG));

	SDL_Rect rect8 = { (Sint16)(PANEL_LEFT + 16), (Sint16)(UI_OFFSET_Y + 427), 250, 35 };
	vecSelConnDlg.push_back(new UiArtTextButton(_(/* UI Element */ "Change Gateway"), nullptr, rect8, UIS_CENTER | UIS_VCENTER | UIS_BIG | UIS_GOLD | UIS_HIDDEN));
	...
	vecSelConnDlg.push_back(new UiArtTextButton(_( /* UI Element - Confirm*/ "OK"), &UiFocusNavigationSelect, rect9, UIS_CENTER | UIS_VCENTER | UIS_BIG | UIS_GOLD));

	SDL_Rect rect10 = { (Sint16)(PANEL_LEFT + 454), (Sint16)(UI_OFFSET_Y + 427), 140, 35 };
	vecSelConnDlg.push_back(new UiArtTextButton(_(/* UI Element - Cancel Button */"Cancel"), &UiFocusNavigationEsc, rect10, UIS_CENTER | UIS_VCENTER | UIS_BIG | UIS_GOLD));
...
void SelconnFocus(int value)
{
	...
	case SELCONN_TCP:
		strncpy(selconn_Description, _(/* UI Element */ "All computers must be connected to a TCP-compatible network."), sizeof(selconn_Description) - 1);
		players = MAX_PLRS;
		break;
	case SELCONN_ZT:
		strncpy(selconn_Description, _(/* UI Element */ "All computers must be connected to a TCP-compatible network."), sizeof(selconn_Description) - 1);
	...
	case SELCONN_LOOPBACK:
		strncpy(selconn_Description, _(/* UI Element */ "Play by yourself with no network exposure."), sizeof(selconn_Description) - 1);
		players = 1;
		break;
	}

	snprintf(selconn_MaxPlayers, sizeof(selconn_MaxPlayers), _(/* UI Element - %d refers to a number. */ "Players Supported: %d"), players);
