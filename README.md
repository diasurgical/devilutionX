...
# How To Play:
 - Copy diabdat.mpq from your CD, or GoG install folder, to the DevilutionX install folder; make sure it is all lowercase. On macOS placing the file in `~/Library/Application Support/diasurgical/devilution` directory would also work, same on Linux for `~/.local/share/diasurgical/devilution/` and on Windows for `C:\Users[username]\AppData\Roaming\diasurgical\devilution`. These path may differ depending on OS version and security setting, so the most reliable recommended location is the directory that contains DevilutionX executable.
